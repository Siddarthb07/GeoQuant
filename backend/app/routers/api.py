...
async def candidates(top_n: int = Query(default=15, ge=1, le=50)) -> list[Candidate]:
    try:
        items = await run_in_threadpool(get_ranked_candidates, top_n)
...
async def candidates_split(per_side: int = Query(default=15, ge=1, le=30)) -> CandidateSplitResponse:
    try:
        payload = await run_in_threadpool(get_ranked_candidates_split, per_side)
...
async def chart(
    ...
    return await run_in_threadpool(get_chart_payload, symbol, bars, timeframe)
