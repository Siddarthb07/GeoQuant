...
NEWS_CACHE: Dict = {"updated_at": None, "daily_series": None}
_CANDIDATE_REFRESH_LOCK = threading.Lock()
_CANDIDATE_REFRESH_RUNNING = False

NEWS_THEME_RULES = [
    {
        "theme": "Energy & Oil",
        "keywords": ["oil", "crude", "opec", "brent", "refinery", "middle east", "shipping lane"],
        "symbols": ["XOM", "RELIANCE.NS", "TMPV.NS"],
        "beta": 1.35,
    },
    {
        "theme": "Rates & Banking",
...
def _get_candidate_cache(top_n: int) -> List[Dict]:
    ...
    if updated is None:
        return []
    if (datetime.now(timezone.utc) - updated).total_seconds() > CANDIDATE_CACHE_TTL_SECONDS:
        return []
    cached = CANDIDATE_CACHE.get("items") or []
    if not cached:
        return []
    return [dict(item) for item in cached[:top_n]]


def _get_news_daily() -> pd.Series:
    updated = NEWS_CACHE.get("updated_at")
    cached_series = NEWS_CACHE.get("daily_series")
    if (
        updated is not None
        and cached_series is not None
        and (datetime.now(timezone.utc) - updated).total_seconds() < 600
    ):
        return cached_series
    news_items = fetch_global_news(limit=200)
    daily_series = daily_news_sentiment_series(news_items)
    NEWS_CACHE["updated_at"] = datetime.now(timezone.utc)
    NEWS_CACHE["daily_series"] = daily_series
    return daily_series


def warm_candidate_cache(top_n: int = 60) -> None:
    _schedule_candidate_refresh(top_n)


def _schedule_candidate_refresh(top_n: int) -> None:
    global _CANDIDATE_REFRESH_RUNNING

    with _CANDIDATE_REFRESH_LOCK:
        if _CANDIDATE_REFRESH_RUNNING:
            return
        _CANDIDATE_REFRESH_RUNNING = True

    def _run() -> None:
        global _CANDIDATE_REFRESH_RUNNING
        try:
            _compute_and_cache_candidates(top_n)
        except Exception:  # noqa: BLE001
            pass
        finally:
            with _CANDIDATE_REFRESH_LOCK:
    ...
    threading.Thread(target=_run, daemon=True, name="candidate-cache-refresh").start()
...
def _build_live_candidate(
    ...
    idx: int,
    daily_bundle: Bundle,
    intraday_bundle: Optional[Bundle],
...
def _compute_and_cache_candidates(top_n: int) -> List[Dict]:
    ...
    target_symbols = symbols[:live_symbol_cap]

    max_workers = min(6, max(1, len(target_symbols)))
    with ThreadPoolExecutor(max_workers=max_workers) as pool:
        futures = {
            pool.submit(
                _build_live_candidate,
                symbol,
                idx,
                daily_bundle,
                intraday_bundle,
...
def get_ranked_candidates(top_n: int = 15, *, force_refresh: bool = False) -> List[Dict]:
    cached = _get_candidate_cache(top_n=top_n)
    if cached and not force_refresh:
        return cached

    if force_refresh:
        return _compute_and_cache_candidates(top_n)

    _schedule_candidate_refresh(max(top_n, 60))
    return _synthetic_candidates(
        top_n=top_n,
        note="Live model signals are still warming up; provisional ranks shown until refresh completes.",
    )
