...
from app.services.signal_service import refresh_bundles
...
def load_pipeline_config(config_path: str, overrides: Optional[Dict] = None) -> ResearchRunConfig:
    ...
    symbols = [resolve_yfinance_symbol(str(x).strip()) for x in symbols]
    # Keep order but drop duplicate resolved tickers.
