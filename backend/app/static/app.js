...
async function renderChart(symbol) {
  $("chart").innerHTML = "<p class='status'>Loading chart data...</p>";
...
function selectCandidate(c) {
  ...
  renderChart(c.symbol).catch((err) => {
    $("chart").innerHTML = `<p class='status short'>Chart failed: ${String(err.message || err)}</p>`;
  });
  setChartAutoRefresh();
}

async function loadCandidates() {
  $("candidateLongTable").innerHTML = "<p class='status'>Loading long candidates...</p>";
  $("candidateShortTable").innerHTML = "<p class='status'>Loading short candidates...</p>";
  try {
    ...
    renderCandidates();
    const warming = [...state.longCandidates, ...state.shortCandidates].some((item) =>
      String(item.rationale || "").toLowerCase().includes("warming up")
    );
    if (warming) {
      setTimeout(() => {
