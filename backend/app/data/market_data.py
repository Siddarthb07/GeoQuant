...
def download_daily_history(symbol: str, start: str, end: str) -> pd.DataFrame:
    ...
    try:
        hist = yf.Ticker(fetch_symbol).history(start=start, end=end, auto_adjust=False, timeout=4)
        if hist.empty:
            return empty
...
def download_intraday_history(symbol: str, period: str = "60d", interval: str = "30m") -> pd.DataFrame:
    ...
    try:
        hist = yf.Ticker(fetch_symbol).history(period=period, interval=interval, auto_adjust=False, timeout=4)
        if hist.empty:
            return empty
        return _normalize_columns(hist)
    except Exception:  # noqa: BLE001
...
def latest_price(symbol: str) -> Optional[float]:
    ticker = yf.Ticker(resolve_yfinance_symbol(symbol))
